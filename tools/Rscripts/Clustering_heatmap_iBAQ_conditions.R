...
my_data %<>%
    dplyr::filter(
        ., `T-test Significant Chlorosis/Vegetative` == "+" |
            `T-test Significant Stage2/Vegetative` == "+" |
            `T-test Significant Stage1Early/Vegetative` == "+" |
            `T-test Significant Stage1Mid/Vegetative` == "+" |
            `T-test Significant LC3/HC` == "+" |
            `T-test Significant LC24/HC` == "+") %>%
    dplyr::filter(., !grepl("chr1", !!as.name(ids_col))) %>%
    dplyr::select(
        ., !!as.name(ids_col),
        tidyselect::starts_with("T-test Difference "))
colnames(my_data) %<>%
    sub("T-test Difference ", "", .)
...
pg_toplot_oa <- pg_toplot %>%
    dplyr::select(., Proteins = id, k_cluster) %>%
    unique(.) %>%
