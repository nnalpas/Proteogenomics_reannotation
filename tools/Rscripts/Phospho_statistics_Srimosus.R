...
wkdir <- "H:/data/Srim_6frame_3rd_analysis/"



### Phosphosites ---------------------------------------------------------

my_data_f <- paste0(wkdir, "MQ_6frame/combined/txt/summary/PhosphoSites_nonredundant.txt")
...
my_reason_f <- paste0(wkdir, "Novel_res_validation/ORF_validation.txt")
...
my_evid_f <- paste0(wkdir, "MQ_6frame/combined/txt/evidence.txt")
...
my_plots[["upset_identification_ref"]] <- UpSetR::upset(
    data = my_phospho_identi_ref %>%
        dplyr::select(., tidyselect::matches("Intensity")) %>%
        lapply(., as.numeric) %>%
        as.data.frame(.),
    nsets = length(
        grep("Intensity", colnames(my_phospho_identi_ref), value = TRUE)),
    nintersects = NA)
...
my_pg_f <- paste0(wkdir, "MQ_6frame/combined/txt/proteinGroups.txt")
...
#        tidyselect::matches("Intensity ")) %>%
#    dplyr::select(
#        ., -tidyselect::matches("^Intensity (L|M|H)$")) %>%
...
my_annot_f <- paste0(wkdir, "EggnogMapper/EggNOG_annotation_2_perseus.txt")
...
my_annot_format <- my_annot %>%
    dplyr::mutate(., Simple_Subcategory = dplyr::case_when(
        ...
        is.na(best_og_Subcategory) | best_og_Subcategory %in% c("", "NA") ~ "Function unknown",
...
my_motifs_annot_count %<>%
    ...
    dplyr::mutate(., Ratio = ifelse(name == "Protein_count", Ratio, NA))
...
my_plots[["Phospho_func_bar"]] <- ggplot(
    ...
    geom_text(stat = "identity", position = "identity", colour = "black", hjust = -0.2, vjust = 1) +
    ggpubr::theme_pubr() +
    scale_fill_manual(
...
my_fasta_f <- paste0(wkdir, "Genome/CP048261_CP048262_prot_sequence_FIXED2.fasta")
...
phospho_ids <- unlist(strsplit(x = my_data$Proteins, split = ";", fixed = ))

my_phospho_prot_list <- split(
    x = phospho_ids,
    f = floor(1:length(phospho_ids)/50))
...
for (x in names(my_phospho_prot_list)) {
    ...
    seqinr::write.fasta(
        ...
        file.out = paste0("Phosphorylated_proteins_50_", x, ".fasta"),
