...
tmp <- params[["pep.loc"]][
    params[["pep.loc"]]$Proteins %in% names(coding.nuc) &
        !is.na(params[["pep.loc"]]$start), ]
cover.nuc <- tmp %>%
    apply(
    X = .,
    MARGIN = 1,
...
text(
    x = 0.025,
    y = sqrt(
        length(unique(unlist(coding.nuc))) / length(chrom.nuc)) + 0.01 - 0.02,
    labels = paste(
        "Protein-coding",
        round(length(unique(unlist(coding.nuc))) / 1000000, 1),
        "Mb", sep = " "),
    col = "green", cex = 1.4, adj = 0)
rect(
    xleft = 0.02,
    ybottom = 0.02,
    xright = sqrt(
...
text(
    x = 0.035,
    y = sqrt(
        length(unique(unlist(exprs.nuc))) / length(chrom.nuc)) + 0.02 - 0.02,
    labels = paste(
        "Expressed protein",
        round(length(unique(unlist(exprs.nuc))) / 1000000, 1),
        "Mb", sep = " "),
    col = "blue", cex = 1.4, adj = 0)
rect(
    xleft = 0.03,
    ybottom = 0.03,
    xright = sqrt(
...
text(
    x = 0.045,
    y = sqrt(
        length(unique(unlist(cover.nuc))) / length(chrom.nuc)) + 0.03 - 0.02,
    labels = paste(
        "Detected peptide",
        round(length(unique(unlist(cover.nuc))) / 1000000, 1),
        "Mb", sep = " "),
    col = "gold", cex = 1.4, adj = 0)
...
DT::datatable(
    data = params[["manual.annot"]] %>%
        dplyr::select(
            ., ORF, ManualAnnotation, PeptideCount, UniProtID,
            FiveNeighbUniprot, ThreeNeighbUniprot),
    rownames = FALSE, width = 500, height = 500)
